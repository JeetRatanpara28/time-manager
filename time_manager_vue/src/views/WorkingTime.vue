...
const formStart = ref(
  fmt(new Date(now.getFullYear(), now.getMonth(), now.getDate(), 9, 0, 0))
);
const formEnd = ref(
  fmt(new Date(now.getFullYear(), now.getMonth(), now.getDate(), 17, 0, 0))
);
...
async function applyFilter() {
  ...
  const qs = new URLSearchParams({
    start: toQueryDateTime(filterStart.value),
    end: toQueryDateTime(filterEnd.value),
  });
  const res = await fetch(`/api/workingtime/${userId}?${qs.toString()}`);
  const json = await res.json();
  resultsList.value = Array.isArray(json)
    ? json
...
async function createWT() {
  ...
  if (
    !(sDate instanceof Date && !isNaN(sDate.getTime())) ||
    !(eDate instanceof Date && !isNaN(eDate.getTime()))
  ) {
...
async function createCustomWT() {
  ...
  const startIso =
    customStart.value.length === 16
      ? `${customStart.value}:00Z`
      : `${customStart.value}Z`;
  const endIso =
    customEnd.value.length === 16
      ? `${customEnd.value}:00Z`
      : `${customEnd.value}Z`;
  // Validation: end must be after start
  const sDate = new Date(startIso);
...
<template>
  <div style="padding: 16px">
    <h2>WorkingTime</h2>
    <div v-if="missingUser" style="color: #b00; margin-bottom: 8px">
      Missing or invalid user. Go back to Users and click a user's WorkingTime
      link.
    </div>
    ...
    <div class="row" style="margin-top: -4px">
      <label
        ><input type="checkbox" v-model="hideOld" /> Hide items older than today
        (UI only)</label
      >
    </div>
    <div class="row">
    ...
    <div class="row" style="margin-top: 4px">
      ...
      <button
        @click="addPreset(new Date().getHours(), new Date().getHours() + 1)"
      >
        Now → +1h
      </button>
    </div>
