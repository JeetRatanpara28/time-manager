...
function setHalfTime() { setDurationHours(4) }
function setFullTime() { setDurationHours(8) }
...
type SessionRow = { in?: string; out?: string; breakIn?: string; breakOut?: string }
const sessionHistory = ref<SessionRow[]>([])

function saveSessionHistory() {
  if (!selectedId.value) return
  localStorage.setItem(sessionStorageKey(selectedId.value), JSON.stringify(sessionHistory.value))
}
...
async function saveWtEdit() {
  ...
  if (!wtEditStart.value || !wtEditEnd.value) { errorMsg.value = 'Please provide start and end'; return }
  ...
  try {
    const res = await fetch(`${PHX_BASE}/api/workingtime/${wtEditId.value}`, {
      ...
      body: JSON.stringify({ working_time: { start: wtEditStart.value, end: wtEditEnd.value } })
    })
    if (!res.ok) {
...
async function deleteWorkingTime(wt: WorkingTime) {
  if (!wt?.id) return
  try {
    const res = await fetch(`${PHX_BASE}/api/workingtime/${wt.id}`, { method: 'DELETE' })
...
function openWtDelete(wt: WorkingTime) {
  ...
  wtDeleteLabel.value = `${fmtDate(wt.start)} ${fmtTime(wt.start)} → ${fmtDate(wt.end)} ${fmtTime(wt.end)}`
...
async function confirmWtDelete() {
  ...
  try {
    const res = await fetch(`${PHX_BASE}/api/workingtime/${wtDeleteId.value}`, { method: 'DELETE' })
    if (!(res.status === 204 || res.ok)) {
      const text = await res.text().catch(()=>'' )
      throw new Error(text || 'Failed to delete working time')
    }
...
type Clock = { id: number; time: string; status: boolean; type?: 'work' | 'break'; break?: boolean }
...
function breakStorageKey(uid: number) { return `breakEvents:${uid}` }
function sessionStorageKey(uid: number) { return `sessionHistory:${uid}` }
function saveBreakLocal() {
  if (!selectedId.value) return
  const payload = { onBreak: onBreak.value, ins: breakIns.value, outs: breakOuts.value }
  localStorage.setItem(breakStorageKey(selectedId.value), JSON.stringify(payload))
}
function loadBreakLocal(uid: number) {
  try {
    const raw = localStorage.getItem(breakStorageKey(uid))
    if (!raw) { onBreak.value = false; breakIns.value = []; breakOuts.value = []; return }
    ...
    onBreak.value = false; breakIns.value = []; breakOuts.value = []
  }
}
...
async function toggleBreak() {
  ...
  if (!isClockedInUi.value) { errorMsg.value = 'You can only use Break when Clocked In'; return }
  ...
  try {
    const res = await fetch(`${PHX_BASE}/api/clocks/${selectedId.value}?type=break`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!res.ok) {
      const text = await res.text().catch(()=>'' )
      errorMsg.value = text || 'Break endpoint not implemented on API; kept local state'
    }
...
async function addWorkingTime() {
  if (!selectedId.value) return
  if (!wtStart.value || !wtEnd.value) { errorMsg.value = 'Please provide start and end'; return }
  ...
  try {
    const res = await fetch(`${PHX_BASE}/api/workingtime/${selectedId.value}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ working_time: { start: wtStart.value, end: wtEnd.value } })
    })
    if (!res.ok) {
      const text = await res.text().catch(()=>'' )
...
async function loadClocks() {
  ...
  try {
    ...
    clocks.value = arr.slice().sort((a, b) => parseDt(a.time) - parseDt(b.time))
    ...
    if (lw && typeof lw.status === 'boolean') {
      ...
      if (workIn.value) {
        ...
        const lastIn = [...clocks.value].filter(c => !isBreakEvent(c) && c.status).reduce((acc, c) => parseDt(c.time) > parseDt(acc?.time) ? c : acc, undefined as any)
        workStartIso.value = lastIn?.time || workStartIso.value
        workEndIso.value = null
      } else {
        // find latest clock-out time
        const lastOut = [...clocks.value].filter(c => !isBreakEvent(c) && !c.status).reduce((acc, c) => parseDt(c.time) > parseDt(acc?.time) ? c : acc, undefined as any)
...
async function toggleClock() {
  ...
  try {
    const res = await fetch(`${PHX_BASE}/api/clocks/${selectedId.value}`, { method: 'POST' })
    if (!res.ok) {
      const text = await res.text().catch(()=>'' )
    ...
    if (!workIn.value) {
      ...
      const afterStart = (t?: string) => !!(t && workStartIso.value && parseDt(t) >= parseDt(workStartIso.value))
    ...
    if (!workIn.value) { onBreak.value = false; saveBreakLocal() }
...
async function confirmDelete() {
  ...
  try {
    const res = await fetch(`${PHX_BASE}/api/users/${deleteId.value}`, { method: 'DELETE' })
    if (!(res.status === 204 || res.ok)) {
      const text = await res.text().catch(()=>'' )
...
const selectedUser = computed<User|undefined>(() => {
  return sortedUsers.value.find(u => u.id === selectedId.value) || users.value.find(u => u.id === selectedId.value)
})
...
const latestWork = computed<Clock | undefined>(() => {
  ...
  return workEvents.reduce((a, b) => (parseDt(a.time) >= parseDt(b.time) ? a : b))
})
...
type ClockRow = { in?: string; out?: string; breakIn?: string; breakOut?: string }
const clockRows = computed<ClockRow[]>(() => {
  ...
  const rows: ClockRow[] = []
  for (let i = 0; i < n; i++) {
    rows.push({ in: ins[i], out: outs[i], breakIn: bins[i], breakOut: bouts[i] })
  }
  return rows
})
...
// We always show all available data across columns in the table
...
async function loadUsers(q: string) {
  ...
  try {
    ...
    const url = `${PHX_BASE}/api/users${params.toString() ? `?${params.toString()}` : ''}`
...
<template>
  <header class="nav" :class="{ detail: showUserDetail }">
    <div class="nav-left">
      <div class="brand" aria-label="Time Manager brand">Time Manager</div>
    </div>

  <!-- Working Time Delete Modal -->
  <div v-if="showWtDeleteModal" class="modal-overlay" @click.self="cancelWtDelete">
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="wtDeleteTitle">
      ...
      <div class="modal-footer">
        <button class="btn small ghost" @click="cancelWtDelete">Cancel</button>
        <button class="btn small danger" @click="confirmWtDelete">Delete</button>
      </div>
    </div>
  </div>
    <div class="nav-center" v-if="showUserDetail">
      <div class="user-chip" :title="selectedUser?.username || selectedUser?.email || ''">
        <div class="avatar-circle">{{ selectedInitials }}</div>
        <div class="user-name">{{ selectedUser?.username || selectedUser?.email }}</div>
      </div>
    </div>
  ...
  <div v-if="showUserDetail" class="subbar">
    <button class="btn small ghost" @click="backToList">Back</button>
  </div>
  ...
  <div v-if="showWtEditModal" class="modal-overlay" @click.self="cancelWtEdit">
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="wtEditTitle">
      <div class="modal-header">
      ...
      <div class="modal-body">
        <div class="field">
          ...
          <input id="wtEditStart" v-model="wtEditStart" class="input" type="datetime-local" />
        </div>
        <div class="field">
          <label for="wtEditEnd">End</label>
          <input id="wtEditEnd" v-model="wtEditEnd" class="input" type="datetime-local" />
        </div>
        <div class="muted" style="font-size:13px">Max duration allowed is 8 hours.</div>
      </div>
      <div class="modal-footer">
      ...
      <div class="layout">
        <div class="left">
          <div class="panel">
            <div class="panel-top">
              <h2>Users</h2>
              <input class="input input-search" v-model="query" @input="onSearch" placeholder="Search by username or email" />
            </div>
            ...
            <div class="table-wrap">
              <table class="table" aria-label="Users table">
                ...
                <tbody>
                  ...
                  <tr v-for="(u, idx) in sortedUsers" :key="u.id" :class="{ 'row-selected': selectedId === u.id }">
                    <td class="muted">{{ idx + 1 }}</td>
                    <td>{{ u.username }}</td>
                    <td class="muted email">{{ u.email }}</td>
                    <td class="actions-cell">
                      <button class="btn small warn" @click="editUser(u)">Edit</button>
                      <button class="btn small danger" @click="openDelete(u)">Delete</button>
                      <button class="btn small info" @click="selectUser(u.id)">Select</button>
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        <aside class="right">
          <div class="panel">
            ...
            <div class="field">
              ...
              <input id="username" v-model="username" class="input" placeholder="e.g., johndoe" />
            </div>
            <div class="field">
              <label for="email">Email</label>
              <input id="email" v-model="email" class="input" type="email" placeholder="e.g., john@example.com" />
            </div>
        ...
        <div class="panel">
          ...
          <div class="field-row">
            <div class="field grow">
              ...
              <input id="wtStart" v-model="wtStart" class="input" type="datetime-local" />
            </div>
            <div class="field grow">
              <label for="wtEnd">End</label>
              <input id="wtEnd" v-model="wtEnd" class="input" type="datetime-local" />
            </div>
          ...
          <div class="row-buttons" style="margin-bottom:12px">
            <button class="btn small ghost preset" @click="setHalfTime">Half time</button>
            <button class="btn small ghost preset" @click="setFullTime">Full time</button>
            <button class="btn small ghost preset" @click="addExtraMinutes(30)">Add +30 min</button>
            <button class="btn small ghost preset" @click="addExtraMinutes(60)">Add +1 hr</button>
          </div>
          <div class="table-wrap">
            <table class="table" aria-label="Working times table">
              ...
              <tbody>
                <tr v-if="wtLoading"><td colspan="6">Loading…</td></tr>
                <tr v-else-if="!workingTimes.length"><td colspan="6">No entries</td></tr>
                <tr v-for="(wt, i) in workingTimes" :key="wt.id">
                  ...
                  <td class="actions-cell">
                    <button class="btn small ghost" @click="selectWorkingTime(wt)">Select</button>
                    <button class="btn small warn" @click="openWtEdit(wt)">Edit</button>
                    <button class="btn small danger" @click="openWtDelete(wt)">Delete</button>
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div class="panel">
          ...
          <div class="stack-vert">
            <div class="status-row">
              <div class="muted">Latest status: <strong>{{ isClockedInUi ? (onBreak ? 'On Break' : 'Clocked-in') : 'Clocked-out' }}</strong></div>
              <div style="display:flex; gap:12px; align-items:center; flex-wrap:wrap">
                <div class="clock-box" aria-label="Daily time tracker">
                  <div class="clock-line"><span>Target</span><strong>{{ msToHMS(targetDurationMs) }}</strong></div>
                  <div class="clock-line"><span>Elapsed</span><strong>{{ msToHMS(elapsedMs) }}</strong></div>
                  <div class="clock-line"><span>Remaining</span><strong>{{ msToHMS(remainingMs) }}</strong></div>
                </div>
                <div class="clock-box" aria-label="Break time tracker">
                  <div class="clock-line"><span>Break Total</span><strong>{{ msToHMS(breakConsumedMs) }}</strong></div>
                  <div class="clock-line"><span>Current Break</span><strong>{{ msToHMS(currentBreakMs) }}</strong></div>
                </div>
              </div>
            </div>
            <div class="row-buttons">
              <button class="btn small info" :class="{ disabled: isClockedInUi && onBreak }" :disabled="isClockedInUi && onBreak" @click="toggleClock">{{ isClockedInUi ? 'Clock Out' : 'Clock In' }}</button>
              <button v-if="isClockedInUi" class="btn small warn" @click="toggleBreak">{{ onBreak ? 'Break Out' : 'Break In' }}</button>
            </div>
            <div class="table-wrap scroll-y">
              <table class="table" aria-label="Clocks table">
                ...
                <tbody>
                  <tr v-if="clockLoading"><td colspan="9">Loading…</td></tr>
                  <tr v-else-if="!currentRow.in && !currentRow.out && !currentRow.breakIn && !currentRow.breakOut"><td colspan="9">No data</td></tr>
                  <tr v-else>
                    <td class="muted">1</td>
            ...
            <div class="panel-top" style="margin-top:12px">
            ...
            <div class="table-wrap scroll-y">
              <table class="table" aria-label="Daily logs table">
                ...
                <tbody>
                  <tr v-if="!dailyLogs.length"><td colspan="6">No logs yet</td></tr>
    ...
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="editTitle">
      ...
      <div class="modal-body">
        ...
        <div class="field">
          ...
          <input id="editEmail" v-model="editEmail" type="email" class="input" />
        </div>
      </div>
      <div class="modal-footer">
        <button class="btn small ghost" @click="cancelEdit">Cancel changes</button>
...
.nav-center { justify-self: center; font-weight: 800; }
...
.btn {
  ...
  text-align: center;
  font-weight: 600;
  transition: background-color .15s ease, border-color .15s ease, transform .05s ease, box-shadow .15s ease, filter .15s ease;
}
.btn.small { height: 32px; min-width: 80px; padding: 0 10px; font-size: 0.9rem; }
.btn.ghost { border-color: var(--border); }
.btn.preset { min-width: 346px; }
.btn.primary { background: var(--brand); color: #001018; border-color: rgba(6,182,212,0.65); box-shadow: 0 0 0 1px rgba(255,255,255,0.06), 0 0 12px rgba(6,182,212,0.25); }
.btn.info { background: var(--brand); color: #001018; border-color: rgba(6,182,212,0.65); }
.btn.warn { background: #f59e0b; color: #111827; border-color: rgba(245,158,11,0.6); }
.btn.danger { background: #ef4444; color: #fff; border-color: rgba(239,68,68,0.7); }
.btn:hover { filter: brightness(0.97); }
.btn.ghost:hover { background: rgba(34, 211, 238, 0.08); }
.btn.primary:hover, .btn.info:hover { filter: brightness(1.05); }
.btn.warn:hover { filter: brightness(1.05); box-shadow: 0 0 0 3px rgba(245,158,11,0.25); }
.btn.danger:hover { filter: brightness(1.05); box-shadow: 0 0 0 3px rgba(239,68,68,0.25); }
.btn:active { transform: translateY(1px); }
.btn:focus-visible { outline: none; box-shadow: 0 0 0 3px rgba(34,211,238,0.35); }

.content { padding: 16px; background: var(--bg); color: var(--fg); min-height: calc(100vh - 60px); }

/* Users page layout */
.page { max-width: 1704px; margin: 0 auto; padding: 8px; }
.layout { display: grid; grid-template-columns: minmax(0,1fr) minmax(260px, 320px); gap: 16px; align-items: start; }
@media (max-width: 1040px) { .layout { grid-template-columns: 1fr; } }

.detail-grid { grid-template-columns: 1fr 1fr; align-items: stretch; }
...
.field-row .grow { flex: 1 1 220px; min-width: 0; }
.actions-inline { display:flex; }
.stack-vert { display:flex; flex-direction:column; gap:12px; flex: 1; min-height: 0; }

/* Prevent panel content from overflowing on small widths */
.panel, .field, .left, .right { min-width: 0; }
...
.input:focus { outline: none; box-shadow: 0 0 0 3px rgba(6,182,212,0.25); }
.input-search { width: 100%; max-width: 360px; flex: 1 1 200px; }

/* Create panel: reduce padding and make inputs and button full-width */
.right .panel { padding: 10px 10px 12px 10px; }
.right .panel .field .input,
.right .panel .btn { width: 100%; min-width: 0; box-sizing: border-box; }

.alert { padding: 10px 12px; border-radius: 10px; margin-bottom: 12px; font-size: 14px; }
.alert.error { background: #1f2937; color: #fecaca; border: 1px solid #991b1b; }
.alert.success { background: #052f35; color: #bbf7d0; border: 1px solid #14532d; }

.table-wrap { overflow:auto; border-radius: 10px; border:1px solid var(--border); max-width: 100%; }
.panel .table-wrap { flex: 1; min-height: 0; }
.table { width: 100%; border-collapse: collapse; }
.table th, .table td { text-align: left; padding: 10px 12px; }
.table thead th { background: #0d1728; color: #cbd5e1; font-size: 13px; letter-spacing: .3px; }
.table tbody tr { border-top: 1px solid var(--border); }
.table tbody tr:nth-child(2n) { background: #0a1322; }
.table td.muted, .muted { color: #9aa3af; }
.table td.email { word-break: break-word; }
...
@media (max-width: 640px) {
  .right .panel .btn { min-width: 0; width: 100%; }
}

/* Make specific tables scroll within a compact area */
.table-wrap.scroll-y { max-height: 260px; overflow: auto; flex: 1; }
@media (max-width: 640px) {
  .table-wrap.scroll-y { max-height: 200px; }
}
...
.clock-box { display:flex; gap: 16px; align-items:center; background:#0b1322; border:1px solid var(--border); border-radius:10px; padding:8px 12px; }
.clock-line { display:flex; gap:8px; align-items:baseline; }
.clock-line span { color: var(--muted); font-size: 13px; }
.clock-line strong { font-variant-numeric: tabular-nums; letter-spacing: .3px; }
...
.modal-overlay { animation: overlayFade .15s ease; }
