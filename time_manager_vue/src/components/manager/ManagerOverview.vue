  ...
  <div class="manager-overview">
    ...
    <div class="kpi-grid">
      ...
      <div class="kpi-card">
        <div class="kpi-icon">✅</div>
        <div class="kpi-content">
          <h3>{{ teamStats.completedToday }}</h3>
          <p>Completed Today</p>
        </div>
      </div>
    </div>
    ...
    <div class="quick-actions-section">
      ...
      <div class="action-grid">
        <div class="action-card primary-card" @click="$emit('navigate', 'team')">
          <div class="card-icon">
            <span>👥</span>
          </div>
        ...
        <div class="action-card secondary-card" @click="$emit('navigate', 'reports')">
          <div class="card-icon">
            <span>📊</span>
          </div>
          <div class="card-content">
            <h4>View Reports</h4>
            <p>Check team performance</p>
          </div>
          <div class="card-arrow">→</div>
        </div>

        <div class="action-card tertiary-card" @click="$emit('navigate', 'clock')">
          <div class="card-icon">
            <span>⏰</span>
          </div>
          <div class="card-content">
            <h4>Clock Control</h4>
            <p>Manage your time tracking</p>
          </div>
          <div class="card-arrow">→</div>
        </div>
      </div>
    ...
    <div class="week-overview-section">
      <div class="week-overview-header">
        ...
        <div class="header-content">
          <h3>Team Overview</h3>
          <p>Weekly team performance insights</p>
        </div>
      </div>
      ...
      <div class="week-stats-grid">
        <div class="week-stat-card">
          ...
          <div class="stat-content">
            <h4>{{ weeklyTeamHours }}h</h4>
            <p>Team Hours This Week</p>
            <span class="stat-trend trend-up">↗️ +15%</span>
          </div>
        </div>
        ...
        <div class="week-stat-card">
          ...
          <div class="stat-content">
            <h4>{{ weeklyActiveMembers }}</h4>
            <p>Active Members</p>
            <span class="stat-trend trend-neutral">→ {{ teamStats.totalMembers }}/{{ teamStats.totalMembers }}</span>
          </div>
        </div>
        ...
        <div class="week-stat-card">
          <div class="stat-icon">📈</div>
          <div class="stat-content">
            <h4>{{ averageTeamHours }}</h4>
            <p>Avg Hours/Member</p>
      ...
      <div class="week-chart-placeholder">
        <div class="chart-header">
          <h4>Team Productivity Trend</h4>
          <p>Hours worked per day this week</p>
        </div>
        <div class="chart-area">
          <div class="chart-bars">
            <div class="chart-bar" v-for="(day, index) in teamWeeklyData" :key="index">
              <div class="bar-fill" :style="{ height: `${(day.hours / 12) * 100}%` }"></div>
    ...
    <div class="alerts-section">
      <div class="alerts-header">
        ...
        <div class="header-content">
          <h3>Team Alerts</h3>
          <p>Important team notifications and issues</p>
        </div>
        <div v-if="teamAlerts.length > 0" class="alerts-count">
          <span class="count-badge">{{ teamAlerts.length }}</span>
        </div>
      </div>

      <div v-if="teamAlerts.length === 0" class="no-alerts">
        <div class="no-alerts-icon">✅</div>
        <h4>Team Status: Good</h4>
        <p>All team members are performing well</p>
      </div>

      <div v-else class="alerts-list">
        <div v-for="alert in teamAlerts" :key="alert.id"
          ...
          <div v-if="alert.action" class="alert-action">
            <button class="alert-btn" @click="handleTeamAlert(alert)">{{ alert.action }}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="recent-activity">
      ...
      <div class="activity-list">
        <div v-for="activity in recentTeamActivities" :key="activity.id"
...
import {
  ...
  teamTodaysLogs,
  timeLogs
} from '@/composables/useManagerStore.js';
...
const getUserName = (userId) => {
  if (!users.value || !users.value.length) return 'Loading...';
...
const getUserRole = (userId) => {
  ...
  const user = users.value.find(u => u.id === userId);
  return user ? user.role.replace('_', ' ') : '';
};

// Weekly team statistics
const weeklyTeamHours = computed(() => {
  if (!timeLogs.value || !timeLogs.value.length) return '0.0';
...
const weeklyActiveMembers = computed(() => {
  if (!timeLogs.value || !timeLogs.value.length) return 0;
...
const averageTeamHours = computed(() => {
  const totalHours = parseFloat(weeklyTeamHours.value);
  const activeMembers = weeklyActiveMembers.value;
  return activeMembers > 0 ? (totalHours / activeMembers).toFixed(1) : '0.0';
});

const topPerformer = computed(() => {
  if (!timeLogs.value || !timeLogs.value.length || !users.value || !users.value.length) return 'Loading...';
...
const teamWeeklyData = computed(() => {
  const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
  return days.map(day => ({
    day,
    hours: Math.floor(Math.random() * 10) + 4 // Mock data: 4-14 hours per day for team
  }));
});

// Team alerts and notifications
const teamAlerts = computed(() => {
  if (!users.value || !users.value.length || !timeLogs.value || !timeLogs.value.length) return [];
  ...
  // Check for team members not clocked in yet (after 9 AM)
  if (currentHour >= 9) {
    users.value.forEach(user => {
      ...
      if (!todaysLog || (!todaysLog.clockIn && todaysLog.status !== 'active')) {
        alertsList.push({
          id: `team-missing-clockin-${user.id}`,
          icon: '⚠️',
          title: 'Team Member Not Clocked In',
          message: `${user.name} hasn't started work yet today`,
  ...
  // Check for team members on very long breaks (>2 hours)
  timeLogs.value.forEach(log => {
    if (log.date === today && log.breakStart && !log.breakEnd) {
      ...
      const breakDuration = (currentTime - breakStart) / (1000 * 60 * 60); // hours

      if (breakDuration > 2) {
        const user = users.value.find(u => u.id === log.userId);
        if (user) {
          alertsList.push({
            id: `long-team-break-${log.id}`,
            icon: '☕',
            title: 'Extended Team Break',
            message: `${user.name} has been on break for ${breakDuration.toFixed(1)} hours`,
            time: formatRelativeTime(breakStart),
            priority: 'medium',
            action: 'Check Status',
            userId: user.id,
            type: 'extended-break'
  ...
  // Check for team members working very late (after 7 PM)
  if (currentHour >= 19) {
    timeLogs.value.forEach(log => {
      if (log.date === today && log.status === 'active' && log.clockIn && !log.clockOut) {
        ...
        if (user) {
          alertsList.push({
            id: `late-team-work-${log.id}`,
            icon: '🌙',
            title: 'Late Team Work',
            message: `${user.name} is still working at ${currentTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}`,
            time: 'After hours',
            priority: 'medium',
            action: 'Remind to Clock Out',
            userId: user.id,
            type: 'late-work'
          });
        }
  ...
  // Check for low team productivity (less than 50% active during work hours)
  if (currentHour >= 10 && currentHour <= 17) {
    const activeMembers = timeLogs.value.filter(log =>
      log.date === today && log.status === 'active' && log.clockIn && !log.clockOut
    ).length;

    const totalMembers = teamStats.value.totalMembers;
    const activePercentage = totalMembers > 0 ? (activeMembers / totalMembers) * 100 : 0;

    if (activePercentage < 50) {
      alertsList.push({
        id: 'low-team-productivity',
        icon: '📉',
        title: 'Low Team Productivity',
        message: `Only ${activeMembers}/${totalMembers} team members are active (${activePercentage.toFixed(0)}%)`,
  ...
  // Sort alerts by priority (high -> medium -> low) and return top 6
  return alertsList
    .sort((a, b) => {
...
// Handle team alert actions
const handleTeamAlert = (alert) => {
  console.log('Handling team alert action:', alert);

  if (alert.type === 'missing-clockin' || alert.type === 'late-work' || alert.type === 'extended-break') {
    // Navigate to team management or user details
    // emit('navigate', 'team');
    console.log(`Navigate to team member details for user ${alert.userId}`);
  } else if (alert.type === 'low-productivity') {
    // Navigate to monitoring view
    // emit('navigate', 'monitoring');
    console.log('Navigate to team monitoring view');
  }
};

// Recent team activities
const recentTeamActivities = computed(() => {
  if (!timeLogs.value || !timeLogs.value.length || !users.value || !users.value.length) return [];
...
@media (max-width: 768px) {
  ...
  .alerts-header .header-content h3 {
    font-size: 1.5rem;
  }
